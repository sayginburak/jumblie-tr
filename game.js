...
function handleKeydown(event) {
	...
	if (key.length === 1 && (key >= "a" && key <= "z" || ["ş", "ç", "ğ", "ı", "ü", "ö"].includes(key))) {
		const indexes = [];
...
function playedToday() {
	if (hasPlayedToday()) {
		...
		document.getElementById("pause").remove();
		document.getElementById("message").textContent =
			"Bugün tüm kelimeleri buldunuz! Yarın tekrar oynamak için sabırsızlanıyorum!";
		document.getElementById("submit").remove();
		document.getElementsByClassName("working-word")[0].remove();
		document.getElementsByClassName("submission")[0].remove();
...
document.getElementById("stats").addEventListener("click", () => {
	...
	currentStreakElement.textContent = `${currentStreak} gün`;
	longestStreakElement.textContent = `${longestStreak} gün`;
	totalDaysPlayedElement.textContent = `${totalDaysPlayed} oyun`;
